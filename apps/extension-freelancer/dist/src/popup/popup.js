...
const MODE_HINTS = {
  manual: "Manual — AI drafts; you copy & send on Freelancer.",
  auto: "Auto — AI drafts and the extension sends replies on Freelancer.",
};
...
async function triggerBriefDecision(action) {
  ...
  try {
    await postBriefDecision(projectId, action);
...
async function recheckMembership() {
  ...
  try {
    ...
    if (res?.ok && !res.payment_required) {
      ...
      statusEl.textContent = res?.error || "Open the Freelancer chat, then recheck.";
    }
  } finally {
    btn.disabled = false;
  }
...
$("copy-btn").addEventListener("click", async () => {
  ...
  setStatus(
    mode === "auto"
      ...
      : "Copied — paste and send on Freelancer",
  );
  const last = await get(KEYS.lastDraft);
